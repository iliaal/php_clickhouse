...
- Numeric `Array(Int* | UInt* | Float* | Decimal* | Bool)` typed query parameters reject a non-numeric string or `Stringable`-object element (e.g. `"1,2,3"`) instead of splicing it raw into the SQL array literal, where it would corrupt element arity or inject punctuation. Only `int` / `float` / `bool` elements, which the extension formats itself, skip the numeric-literal gate.
...
- A throwing `__toString()` on the `host` config value passed to the constructor now surfaces as a PHP exception instead of aborting the process (the value was coerced ahead of the constructor's C++ try block).
- Out-of-range date/time inserts are rejected instead of silently wrapping: `Date` (1970-01-01 .. 2149-06-06), `Date32` (1900-01-01 .. 2299-12-31), and `DateTime` (1970-01-01 .. 2106-02-07) now range-check the epoch before clickhouse-cpp narrows it into the column storage, and the `DateTime64` string path guards its tick multiply against `int64` overflow (matching the integer path). Previously `Date "3000-01-01"` stored `2102-11-05` and `DateTime "1960-..."` wrapped to 2096.
- `Decimal` inserts reject a value that exceeds the column's precision or has more fractional digits than its scale, instead of silently storing a truncated or over-range value that the server's own `VALUES` parser would reject (`Decimal(5,2)` no longer accepts `1000.00`).
- `Array(String)` (and other quoted-element array) typed query parameters now round-trip single quotes and backslashes correctly. The array literal is sent as a bound parameter whose reader treats backslashes literally and closes an element on the first quote, so quotes are doubled (SQL style) and backslashes left literal; the previous scheme rejected `"it's"` and doubled `"c\d"`.
